"""
이베스트 증권에서 제공하는 api 구동모듈
** 주의사항 **
 프로세스당 1개의 클래스만 등록하여 사용해야함
"""
...
class XAEvents:
    """
        Event handler class
    """
    instance = None # 프로그램 인스턴스

    __events = dict(
        OnLogin=None, # 서버와의 로그인이 끝나면 발생 (args: code, msg)
        OnDisconnect=None, # 서버와의 연결이 끊어졌을때 발생
        OnReceiveData=defaultdict(dict), # 서버로부터 데이터를 수신했을때 발생
        OnReceiveMessage=None, # 서버로부터 메시지를 수신했을 때 발생
        OnReceiveChartRealData=None, # 차트 지표데이터 조회시, 실시간 자동 등록으로 "1"로 했을 경우 발생
        OnReceiveRealData=defaultdict(dict), # 서버로부터 데이터를 수신했을때 발생
        OnreceiveLinkData=None # HTS로부터 연동 정보를 수신했을때 발생
    )
    ...
    def on(event, code=None):
        ...
        def decorator(func):
            ...
            if event in ['OnReceiveData', 'OnReceiveRealData']:
                caller = func.__name__.split('on_',1)[1] #on_뒤에 나오는 함수호출
                XAEvents.__events[event][code][caller] = func
            else:
                XAEvents.__events[event] = func
        return decorator

    def trigger(self, event):
        #print(XAEvents.__events)
        """ Call appropriate event handlers when event comes from the server """
        def triggered(*args):
            if event in ['OnReceiveData', 'OnReceiveRealData']:
                # 현재 진행중인  tr에 등록된 함수명과 동일한  핸들러 이벤트를 실행함
                handler = XAEvents.__events[event][args[1]][self.instance.state]
            else:
                handler = XAEvents.__events[event]
            handler(self.instance, *args[1:])
        return triggered
...
class Session:
    ...
    def __init__(self, demo=False):
        ...
        XAEvents.instance.state = inspect.stack()[1][3]
...
class Query:
    def __init__(self, trcode):
        ...
        self.tr = tr[trcode] #현재 처리중인 tr 오브젝트
        XAEvents.instance.state = inspect.stack()[1][3]
        #com object 생성
        pythoncom.CoInitialize()
    ...
    def get_tr_count_per_sec(self, trcode):
        """ TR의 초당 전송 가능 횟수 """
        return self.xing.GetTCountPerSec(trcode)
    ...
    def get_tr_count_limit(self, trcode):
        """
        TR의 10분당 제한 건수
        """
        return self.xing.GetTRCountLimit(trcode)
